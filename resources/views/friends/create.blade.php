<x-app-layout>
    ...
    <div class="p-4 md:p-6 bg-surface border border-border-color rounded-lg">
        <div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <x-button variant="outline" href="{{ url()->previous() }}" class="w-full sm:w-auto justify-center">
                &lt; {{ __('Back to Network') }}
            </x-button>
        </div>

        @if($errors->any())
        <div class="mb-6 bg-red-900/50 border-l-4 border-red-500 text-red-300 p-4 rounded-r-lg font-mono text-sm"
            role="alert">
        ...
        <form method="POST" action="{{ route('friends.store') }}">
            ...
            <div class="pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-forms.input type="text" id="codename" name="codename" :value="old('codename')" />
                    </div>

                    {{-- Category Dropdown (Pakai Component Kita) --}}
                    <div>
                        <x-forms.select label="> {{ __('ASSET CATEGORY') }}" name="category" id="category"
                            :options="$categories" :selected="old('category')"
                            placeholder="{{ __('-- Select Category --') }}" :searchable="true" />
                    </div>
                </div>
            </div>
            ...
            <div class="relative flex py-2 items-center">
                ...
                <span class="flex-shrink-0 mx-4 text-secondary font-bold tracking-widest bg-surface px-2">{{ __('OR')
                    }}</span>
                <div class="flex-grow border-t border-border-color"></div>
            </div>
                    ...
                    Karena kita butuh <optgroup>, kita gunakan Native Select tapi dengan class .form-control
                        ...
                        <div class="relative font-mono w-full">
                            <select id="target_entity" name="target_entity"
                                ...
                                <optgroup class="bg-surface text-primary font-bold"
                                    label="{{ __('OPERATIVES (USERS)') }}">
                                ...
                                <optgroup class="bg-surface text-primary font-bold"
                                    label="{{ __('EXISTING ASSETS (FRIENDS)') }}">
