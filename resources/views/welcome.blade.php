    ...
    <div class="relative min-h-screen">
            ...
            <div class="container mx-auto flex justify-between items-center">
                ...
                @auth
                    ...
                    <a href="{{ route('dashboard') }}"
                        class="px-4 py-2 border border-primary text-primary hover:bg-primary hover:text-base transition-colors font-bold text-sm">
                        > ENTER DASHBOARD
                    </a>
                @else
        ...
        <main>
            <section class="min-h-screen flex items-center justify-center text-center p-4">
                <div x-data="{ showCursor: true }" x-init="setInterval(() => showCursor = !showCursor, 600)">
                    ...
                    <p class="mt-4 max-w-2xl mx-auto text-sm sm:text-base">
                        A secure, encrypted, and decentralized archive for sensitive field operations and agent network
                        management.
                    </p>
                ...
                <div class="container mx-auto">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                        <div class="border border-border-color p-6 rounded-lg">
                            ...
                            <p class="text-sm">Securely manage operative profiles, designations, and clearance levels.
                                On-demand deployment and extraction protocols.</p>
                        </div>
                        <div class="border border-border-color p-6 rounded-lg">
                            <h3 class="text-xl font-bold text-white mb-2">> MISSION DIRECTIVES</h3>
                            <p class="text-sm">Create, assign, and monitor multi-phase operations. Real-time status
                                tracking from inception to archival.</p>
                        </div>
                        <div class="border border-border-color p-6 rounded-lg">
                            <h3 class="text-xl font-bold text-white mb-2">> ASSET NETWORK</h3>
                            <p class="text-sm">Map and visualize hierarchical relationships between operatives,
                                informants, and key assets in the field.</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="py-20 text-center p-4">
                <div class="container mx-auto">
                    <h2 class="text-2xl font-bold text-white">Your mission, should you choose to accept it, begins here.
                    </h2>
                    <p class="mt-4 mb-8 max-w-xl mx-auto">This terminal is the gateway. Verify your identity to proceed.
                    </p>

                    @auth
                        {{-- Tombol ini tampil JIKA pengguna SUDAH LOGIN --}}
                        <a href="{{ route('dashboard') }}"
                            class="inline-block px-8 py-4 bg-black text-primary transition-colors font-bold tracking-widest text-lg hover:bg-primary-hover">
                            > PROCEED TO DASHBOARD
                        </a>
                    @else
                        {{-- Tombol ini tampil JIKA pengguna BELUM LOGIN --}}
                        <a href="{{ route('login') }}"
                            class="px-8 py-4 bg-black text-primary transition-colors font-bold tracking-widest text-lg hover:bg-primary-hover">
                            > ACCESS TERMINAL
                        </a>
                    @endauth
                </div>
        ...
        <footer class="text-center p-4 border-t border-border-color bg-surface">
            <p class="text-xs text-secondary/50">&copy; <span class="font-bold"> 2025 - {{ date('Y') }} </span>
                Directorate Internal Systems // All communications are monitored and encrypted.</p>
        </footer>
    </div>
</body>
