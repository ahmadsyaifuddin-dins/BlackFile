 ...
 <x-app-layout title="{{ __('System Settings') }}">
     ...
     @if (session('success'))
         <div class="bg-green-900/50 border border-primary text-primary px-4 py-3 rounded relative mb-6 font-mono text-sm"
             role="alert">
         ...
         <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             ...
             <div class="bg-surface border border-border-color p-6 font-mono h-full">
                 <h2 class="text-lg font-bold text-primary border-b border-border-color pb-2 mb-4">>
                     {{ __('Language Setting') }}</h2>

                 <x-forms.select label="{{ __('Select Terminal Language:') }}" name="locale" :options="$localeOptions"
             ...
             <div class="bg-surface border border-border-color p-6 font-mono h-full">
                 <h2 class="text-lg font-bold text-primary border-b border-border-color pb-2 mb-4">>
                     {{ __('Data Display Setting') }}</h2>

                 <x-forms.select label="{{ __('Entries Per Page:') }}" name="per_page" :options="$perPageOptions"
             ...
             <div class="bg-surface border border-border-color p-6 font-mono h-full">
                 ...
                 <x-forms.select label="{{ __('Select Terminal Theme:') }}" name="theme" :options="$themeOptions"
             ...
             {{-- Panel 3: Layout Notifikasi (BARU) --}}
             <div class="bg-surface border border-border-color p-6 font-mono h-full">
                 <h2 class="text-lg font-bold text-primary border-b border-border-color pb-2 mb-4">>
                     {{ __('HUD Layout') }}</h2>
