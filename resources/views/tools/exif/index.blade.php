<x-app-layout title="EXIF INTEL // OSINT">
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 min-h-screen flex flex-col gap-6">
        ...
        <div class="flex justify-between items-end border-b border-cyan-900/50 pb-4">
            ...
            <x-button variant="outline" href="{{ route('tools.index') }}"
                class="text-xs border-cyan-900 text-cyan-700 hover:bg-cyan-600 hover:border-cyan-600 hover:text-black transition-all">
                [ RETURN TO BASE ]
            </x-button>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-4 space-y-4">
                <div class="bg-surface border border-cyan-900/30 rounded-lg p-1 shadow-[0_0_20px_rgba(8,145,178,0.1)]">
                    <div class="border-2 border-dashed border-cyan-900/50 rounded-lg p-8 text-center hover:bg-cyan-900/10 transition-colors relative"
                        id="dropZone">

                        <input type="file" id="imageInput" accept="image/jpeg,image/jpg,image/tiff"
                        ...
                        <div id="filePreview" class="hidden">
                            <img id="previewImg" src=""
                                class="max-h-40 mx-auto rounded border border-cyan-500/50 shadow-lg mb-3 object-contain">
                            <p id="fileName" class="text-xs text-cyan-300 font-mono truncate"></p>
                            <button type="button" onclick="resetUpload()"
                                class="mt-2 text-[10px] text-red-400 hover:text-red-300 underline z-20 relative">Remove
                                File</button>
                        </div>
                    </div>
                </div>

                <button id="btnAnalyze" onclick="analyzeImage()"
                    class="cursor-pointer w-full bg-cyan-900/20 border border-cyan-500 text-cyan-400 font-bold py-3 rounded hover:bg-cyan-500 hover:text-black transition-all disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wider text-sm">
                ...
                <div class="bg-black/30 border-l-2 border-amber-500 p-3 rounded-r text-[10px] text-gray-400 font-mono">
                    ...
                    Images from WhatsApp, Facebook, or Instagram usually have metadata STRIPPED (Cleaned). Use
                    "Document" mode or original camera files for best results.
                </div>
            </div>

            <div
                class="lg:col-span-8 bg-black border border-cyan-900/50 rounded-lg p-4 font-mono text-sm relative overflow-hidden min-h-[400px]">
                <div
                ...
                <div class="relative z-10 h-full flex flex-col">
                    ...
                    <div id="loadingState"
                        class="hidden flex-grow flex flex-col items-center justify-center text-cyan-400">
                    ...
                    <div id="resultContent" class="hidden space-y-6 overflow-y-auto pr-2 max-h-[500px]">
                        ...
                        <div id="gpsSection" class="hidden">
                            <h4 class="text-xs font-bold text-black bg-cyan-500 inline-block px-2 py-0.5 mb-2">Target
                                Location Detected</h4>
                            <div
                                class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-cyan-900/10 p-4 border border-cyan-500/30 rounded">
                                <div>
                                    <div class="mb-2">
                                ...
                                <div class="flex items-center justify-center">
                                    <a id="mapLink" href="#" target="_blank"
                                        class="group flex flex-col items-center justify-center w-full h-full border border-dashed border-cyan-600 hover:bg-cyan-900/30 transition-colors rounded p-4">
                                        <i
                                            class="fa-solid fa-map-location-dot text-3xl text-cyan-500 mb-2 group-hover:scale-110 transition-transform"></i>
                        ...
                        <div>
                            <h4 class="text-xs font-bold text-cyan-500 border-b border-cyan-900 inline-block mb-2">
                                Device Signature</h4>
                        ...
                        <div>
                            <h4 class="text-xs font-bold text-cyan-500 border-b border-cyan-900 inline-block mb-2">File
                                Attributes</h4>
                            <div class="grid grid-cols-2 gap-4 text-xs">
                                <div>
            ...
            async function analyzeImage() {
                ...
                try {
                    ...
                    if (data.gps) {
                        ...
                        noGpsMsg.className =
                            'text-xs text-amber-500 mb-4 font-mono border border-amber-900/30 bg-amber-900/10 p-2 rounded';
