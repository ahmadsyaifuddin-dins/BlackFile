@push('scripts')
    <script>
        ...
        const SITES = [{
            ...
            {
                name: 'Medium',
                url: 'https://medium.com/@{name}',
                icon: 'fa-medium',
                color: 'text-white'
            },
            {
                name: 'Spotify',
                url: 'https://open.spotify.com/user/{name}',
                icon: 'fa-spotify',
            ...
            {
                name: 'Steam',
                url: 'https://steamcommunity.com/id/{name}',
                icon: 'fa-steam',
                color: 'text-blue-300'
            },
            {
            ...
            {
                name: 'Discord',
                url: 'https://discord.com/users/{name}',
                icon: 'fa-discord',
            ...
            {
                name: 'Threads',
                url: 'https://www.threads.net/@{name}',
                icon: 'fa-threads',
                color: 'text-gray-300'
            },
            {
                name: 'CodePen',
                url: 'https://codepen.io/{name}',
                icon: 'fa-codepen',
                color: 'text-white'
            },
            {
    ...
    `).join('');
        ...
        async function clearHistory() {
            ...
            // Panggil window.agentConfirm(Title, Message, ButtonYes, ButtonNo)
            const isConfirmed = await window.agentConfirm(
                'CONFIRM DELETION',
                'Are you sure you want to purge all search logs? This action cannot be undone.',
                'PURGE DATA', // Custom text tombol YES
                'CANCEL' // Custom text tombol NO
            );
            ...
            if (isConfirmed) {
                ...
                // Munculkan alert sukses setelah confirm selesai
                window.agentAlert('success', 'MEMORY WIPED', 'Search history log has been purged successfully.');
            }
        }
        ...
        function exportResults() {
            if (currentResults.length === 0) {
                window.agentAlert('warning', 'EXPORT FAILED', 'No data available to export. Initiate scan sequence first.');
                return;
            }
            ...
            window.agentAlert('success', 'DATA EXTRACTED',
                `File osint_${username}_${timestamp}.json downloaded successfully.`);
        }
        ...
        async function startScan() {
            ...
            if (!inputRaw) {
                window.agentAlert('error', 'MISSING INPUT', 'Target identifier required. Please enter a username.');
                return;
            }
            ...
            btn.innerHTML =
                '<i class="fa-solid fa-circle-notch fa-spin"></i> <span class="hidden sm:inline">Scanning...</span><span class="sm:hidden">...</span>';
            ...
            btn.innerHTML =
                '<i class="fa-solid fa-satellite-dish"></i> <span class="hidden sm:inline">Start Scan</span><span class="sm:hidden">Scan</span>';
    ...
    `;
        ...
        async function checkSite(site, username, logElement, gridElement) {
            ...
            const googleDorkUrl =
                `https://www.google.com/search?q=${encodeURIComponent(`site:${domain} ${queryForDork}`)}`;
            ...
            `;
                ...
                if (data.status === 'FOUND') {
                    logItem.innerHTML =
                        `<span class="text-primary">✓</span> <span class="text-green-400 font-bold">${site.name}</span> - Account found`;
                    logItem.className = 'text-green-400';
                    renderCard('FOUND', 'text-primary bg-primary/10 border-primary/20',
                    ...
                    renderCard('MANUAL', 'text-amber-500 bg-amber-500/10 border-amber-500/20',
                        'border-amber-500/30 hover:border-amber-500', true);
                    syncMobileLog();
                    return {
                        platform: site.name,
                        status: 'MANUAL_CHECK',
