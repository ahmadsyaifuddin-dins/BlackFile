...
<div x-data="creditsForm({ initialCredits: {{ $credits ?? '[]' }} })" class="text-white">
            ...
            <div class="pl-4 border-l-2 border-gray-700">
                ...
                <template x-for="(name, nameIndex) in credit.names" :key="nameIndex">
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mb-2">
                        <input type="text" :name="'credits[' + creditIndex + '][names][' + nameIndex + ']'"
                            x-model="credit.names[nameIndex]" required placeholder="Full Name"
                            class="flex-grow bg-gray-800 border-gray-600 rounded-md shadow-sm text-white">
                        <button @click="removeName(creditIndex, nameIndex)" type="button"
                ...
                <button @click="addName(creditIndex)" type="button"
                    class="text-xs text-primary hover:underline cursor-pointer ">+ Add Name</button>
            </div>
    ...
    selectedMusic: '{{ (isset($musicPath) && Str::startsWith($musicPath, 'music/default')) ? $musicPath : '' }}'
        ...
        <div x-show="musicSource === 'default'" x-transition>
            ...
            <select name="default_music_path" id="default_music_path" x-model="selectedMusic"
                class="mt-1 block w-full bg-gray-800 border-gray-600 rounded-md text-white">
