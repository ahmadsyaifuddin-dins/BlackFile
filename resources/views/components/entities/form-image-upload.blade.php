        ...
        <div>
                ...
                <div x-show="filePreviews.length > 0" class="mt-4">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
                        <template x-for="(preview, index) in filePreviews" :key="index">
                            <div class="relative group">
                                <img :src="preview" class="w-full h-32 object-cover border-2"
                                    :class="selectedThumbnail === 'new_index_' + index ? 'border-primary' :
                                        'border-border-color'">

                                <div class="mt-2 flex justify-center">
                                    <x-forms.radio {{-- Gunakan x-bind: untuk atribut Alpine --}} x-bind:id="'thumb_new_' + index"
                                        x-model="selectedThumbnail" x-bind:value="'new_index_' + index">
                                        Set Thumbnail
                                    </x-forms.radio>
                                </div>
                            </div>
            ...
            <div x-show="tab === 'link'" x-transition style="display: none;">
                ...
                <x-forms.input type="text" name="image_url" id="image_url"
                    placeholder="https://... Paste image URL here" x-model="urlInput" @input="handleUrlChange"
                    class="mt-1 w-full bg-transparent border-0 border-b-2 border-border-color focus:border-primary focus:ring-0 text-white">
                </x-forms.input>
                <p class="text-xs text-secondary mt-2">Note: Only one URL can be added at a time. This will be
                    automatically selected as thumbnail.</p>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
                        <div class="relative group">
                            <img :src="urlInput" class="w-full h-32 object-cover border-2"
                                ...
                                x-on:error.once="$event.target.src = '{{ asset('placeholder-error.png') }}'">
                            {{-- Fallback jika URL error --}}
                            <div class="mt-2 text-center">
                                <x-forms.radio id="thumb_url" x-model="selectedThumbnail" value="new_url"
                                    class="w-4 h-4 bg-base border-border-color text-primary focus:ring-primary mr-2">
                                    Set Thumbnail
                                </x-forms.radio>
                            </div>
