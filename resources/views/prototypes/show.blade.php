        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gray-900 border border-gray-700 shadow-sm sm:rounded-lg overflow-hidden">
                ...
                @if($prototype->cover_image_path)
                    <div class="bg-black">
                        <img src="{{ asset($prototype->cover_image_path) }}" alt="Cover image for {{ $prototype->name }}" class="w-full h-64 object-cover object-center">
                    </div>
                @endif
                    ...
                    <div class="bg-gray-800 p-4 rounded-lg border border-gray-700 h-fit">
                        ...
                        <div class="space-y-3 text-sm">
                            ...
                            <div>
                                ...
                                <p>{{ $prototype->start_date ? $prototype->start_date->locale('id')->translatedFormat('d M Y, H:i') : 'N/A' }}</p>
                            </div>
                            <div>
                                <p class="text-gray-500">> DEPLOYMENT END</p>
                                <p>{{ $prototype->completed_date ? $prototype->completed_date->locale('id')->translatedFormat('d M Y, H:i') : 'N/A' }}</p>
                            </div>
                             <div>
                                <p class="text-gray-500">> CODE VAULT</p>
                                @if($prototype->repository_url)
                                    <a href="{{ $prototype->repository_url }}" target="_blank" class="text-indigo-400 hover:underline break-all">
                            ...
                            <div>
                                <p class="text-gray-500">> LIVE DEPLOYMENT</p>
                                 @if($prototype->live_url)
                                    <a href="{{ $prototype->live_url }}" target="_blank" class="text-indigo-400 hover:underline break-all">
