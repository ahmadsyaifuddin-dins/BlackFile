        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                <h1 class="text-4xl font-bold text-primary tracking-widest font-mono text-glow">
                    DOSSIER: {{ $entity->codename ?? $entity->name }}
                </h1>
            </div>
            <div class="flex flex-col md:flex-row gap-3">
                <a href="{{ route('entities.index') }}"
                    class="font-mono text-center border border-border-color px-4 py-2 hover:bg-surface-light">&lt;
                    BACK</a>
                <a href="{{ route('entities.edit', $entity) }}"
                    class="font-mono text-center border border-border-color w-full md:w-auto px-3 md:px-4 py-2 text-sm md:text-base hover:bg-surface-light">
                    <span class="md:hidden">> EDIT</span>
                    <span class="hidden md:inline">> EDIT FILE</span>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            ...
            <div class="space-y-4">
                ...
                @php
                // Cek apakah path adalah URL lengkap atau path lokal
                $imagePath = Illuminate\Support\Str::startsWith($image->path, 'http')
                ? $image->path
                : asset('uploads/' . $image->path);
                @endphp
                <div>
                    <img src="{{ $imagePath }}" alt="{{ $image->caption ?? 'Entity Image' }}"
                        class="w-full h-auto object-cover border-2 border-border-color hover:border-primary transition-all duration-200">
                    @if($image->caption)
                    <p class="text-xs text-secondary mt-2 text-center bg-surface py-1 font-mono">{{ $image->caption }}
                    </p>
                    @endif
                </div>
                @endforeach
            ...
            @else
            <div
                class="w-full h-48 bg-surface flex items-center justify-center border-2 border-dashed border-border-color mt-2">
