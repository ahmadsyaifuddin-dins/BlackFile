...
<body class="antialiased selection:bg-red-900 selection:text-white">
    ...
    <div class="relative z-10 min-h-screen flex flex-col items-center">
        ...
        <!-- HEADER SECTION -->
        <header class="w-full max-w-5xl mx-auto pt-12 pb-8 px-6 border-x border-gray-900 bg-[#050505] relative">

            <!-- TOMBOL ADMIN (PINDAH KESINI) -->
            <!-- Mobile: Relative (mendorong konten ke bawah). Desktop: Absolute di Kanan Atas (agar tidak menabrak teks kiri) -->
            @auth
                <div
                    class="w-full mb-6 md:mb-0 md:absolute md:top-6 md:right-6 md:w-auto z-50 flex justify-center md:justify-end">
            ...
            <!-- Top Bar -->
            <div
                class="flex flex-col md:flex-row justify-between items-start md:items-end border-b-2 border-red-900/50 pb-4 mb-8 gap-4">
                <div>
                    <span class="block text-xs text-red-600 font-bold tracking-[0.3em] mb-1 animate-pulse">
                        CONFIDENTIAL // DECLASSIFIED
                    </span>
                    <h2 class="text-sm text-gray-500 font-mono">CASE ID: {{ $archive->case_code }}</h2>
                </div>
                <div class="w-full md:w-auto text-left md:text-right">
                    <a href="{{ route('dark-archives.index') }}"
                        class="text-xs text-gray-600 hover:text-red-500 transition tracking-widest font-mono">
                        [ RETURN TO INDEX ]
                    </a>
                </div>
            </div>
            ...
            <div class="glitch-wrapper mb-6">
                <h1 class="text-3xl md:text-5xl lg:text-6xl font-extrabold text-gray-200 tracking-tighter uppercase leading-none glitch-text font-['Special_Elite']"
            ...
            <!-- Metadata Grid -->
            <div
                class="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-mono text-gray-500 border-b border-gray-900 pb-8">
                <div>
                ...
                <div>
                    ...
                    <span class="text-gray-300">AGENT {{ strtoupper($archive->agent->name ?? 'REDACTED') }}</span>
                </div>
                <div>
        ...
        <!-- ARTICLE CONTENT -->
        <main class="w-full max-w-5xl mx-auto px-6 py-12 border-x border-gray-900 bg-[#050505]">
            <article
                ...
                prose-blockquote:border-l-red-900 prose-blockquote:bg-gray-900/30 prose-blockquote:text-gray-500 prose-blockquote:italic prose-blockquote:p-6
                ">
                {!! $archive->content !!}
            </article>

            <!-- ENGAGEMENT SECTION (RESPECTS) -->
            <div class="mt-24 pt-12 border-t border-gray-900 flex flex-col items-center text-center">
                <div class="mb-8">
                    <p class="text-sm text-gray-600 italic font-serif">"Those who cannot remember the past are condemned
                        to repeat it."</p>
                </div>

                <!-- Respect Button -->
                <button onclick="payRespect({{ $archive->id }})" id="respectBtn"
                    class="group relative inline-flex items-center justify-center px-10 py-4 overflow-hidden font-mono font-bold tracking-widest text-white transition-all duration-300 bg-gray-900 border border-gray-800 hover:bg-black hover:border-red-900 focus:outline-none">
                    ...
                    <span class="relative flex items-center space-x-3">
                        <!-- Candle Icon -->
                        <svg class="w-6 h-6 text-gray-500 group-hover:text-red-500 transition-colors duration-500"
                ...
                <div class="mt-6 font-mono text-xs text-gray-500 space-y-2">
                    ...
                    <div class="flex items-center justify-center space-x-6 pt-4 border-t border-gray-900 w-48 mx-auto">
                        <span class="flex items-center" title="Total Views">
                            <svg class="w-4 h-4 mr-2 text-gray-700" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
        ...
        function payRespect(id) {
            ...
            fetch(`/dark-archives/respect/${id}`, {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        // Update counter with animation logic here if desired
                        countDisplay.innerText = new Intl.NumberFormat().format(data.total);
                    } else {
                        // Re-enable button if needed, but logic says one per session
                    }
