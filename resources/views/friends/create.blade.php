    ...
    <div class="p-4 md:p-6 bg-surface border border-border-color rounded-lg">
        ...
        <form method="POST" action="{{ route('friends.store') }}">
            ...
            <div class="border-b border-border-color pb-6">
                <h3 class="text-base text-primary font-bold mb-4">> OPTION A: REGISTER NEW ASSET</h3>
                <p class="text-sm text-secondary mb-4">// Use this to add a new informant or asset that is not a system
                    user.</p>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label for="category" class="block text-primary text-sm">> ASSET CATEGORY</label>
            ...
            <div>
                <h3 class="text-base text-primary font-bold mb-4">> OPTION B: CONNECT TO EXISTING ENTITY</h3>
                <p class="text-sm text-secondary mb-4">// Use this to link to another registered operative or an
                    existing asset.</p>
                <div>
                    ...
                    <select id="target_entity" name="target_entity"
                        ...
                        <optgroup label="OPERATIVES (USERS)">
                            ...
                            <option value="user-{{ $user->id }}">{{ $user->codename }} ({{ $user->role->alias }})
                            </option>
                            @endforeach
                        </optgroup>
