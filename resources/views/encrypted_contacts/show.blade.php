<x-app-layout :title="'View Contact: ' . $contact->codename">
    ...
    <div x-data="{ unhidden: true }" class="bg-surface border border-border-color p-6 font-mono space-y-6">
        <div class="flex justify-between items-center border-b border-border-color pb-2 mb-4">
            ...
            {{-- Tombol Hide/Unhide --}}
            <button @click="unhidden = !unhidden"
                class="border border-border-color text-secondary px-3 py-1 text-xs hover:border-primary hover:text-primary">
        ...
        <img src="{{ asset($contact->profile_photo_path) }}" alt="{{ $contact->codename }}"
            class="w-32 h-32 object-cover border-2 border-border-color flex-shrink-0">
        @else
        <div
            class="w-32 h-32 bg-base flex items-center justify-center border-2 border-dashed border-border-color flex-shrink-0">
            <span class="text-secondary font-mono text-sm">[NO PHOTO]</span>
        </div>
        @endif
    ...
    <div class="max-w-xl mx-auto bg-surface border-2 border-yellow-500/50 p-6 font-mono">
        <h2 class="text-lg font-bold text-yellow-400 border-b border-yellow-500/30 pb-2 mb-4">> VAULT ACCESS REQUIRED
        </h2>
        <p class="text-sm text-secondary mb-4">Enter your Master Password to decrypt and view the data for this contact.
        </p>
        <form action="{{ route('encrypted-contacts.unlock', $contact) }}" method="POST" class="space-y-4">
            ...
            <div class="flex justify-end pt-2">
                <button type="submit"
                    class="bg-primary text-black font-bold py-2 px-6 hover:bg-primary-hover transition-colors">
                    DECRYPT
                </button>
            </div>
        </form>
